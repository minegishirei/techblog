...
アーキテクチャスタイルに幾つかのケースがあるのと同じように、
リファクタリングの手法にも明確な6段階の手順が存在します。

今回の記事の目的はリファクタリングを行う2通りの方法について説明します。

- [大規模なリファクタリングを行う前に](#大規模なリファクタリングを行う前に)
- [リファクタリングでやってはいけないこと-象の移行アンチパターン](#リファクタリングでやってはいけないこと-象の移行アンチパターン)
    - [コンポーネントベース分解概要](#コンポーネントベース分解概要)
    - [戦術的フォーク](#戦術的フォーク)
  - [コードベースが分解可能であるとは](#コードベースが分解可能であるとは)
    - [コンポーネントへの入力と出力の数から判断する](#コンポーネントへの入力と出力の数から判断する)
    - [抽象度](#抽象度)
- [アーキテクチャを分解する](#アーキテクチャを分解する)
  - [コンポーネントベース分解](#コンポーネントベース分解)
  - [戦術的フォーク](#戦術的フォーク-1)
    - [戦術的フォークのデメリット](#戦術的フォークのデメリット)
  - [備考](#備考)
...
[詳しい内容はこちらの記事より参照してください-リファクタリングをするべき5つの理由](https://minegishirei.hatenablog.com/entry/2023/02/02/162802)
